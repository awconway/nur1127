...
### <i class="fad fa-calculator"></i> Difference in means is 1.82
...
```{r cache=TRUE, echo=FALSE, fig.height=5, fig.retina=8}
set.seed(2)
shuffle1 <- data %>% 
  specify(leadership ~ degree) %>%
  hypothesize(null = "independence") %>%
...
ggplot2::ggplot(aes(stat)) +
    scale_y_continuous( limits = c(0,1))+
    scale_x_continuous(limits = c(-2.5,2.5), breaks = c(seq(-2,2, by=1)))+
  theme_minimal()+
  labs(x = "Difference in mean leadership score between groups", y = NULL)+
  geom_vline(xintercept = 0, size = 1, color = "black")+
