...
Let's revisit the last exercises where we looked into a hypothetical example of a sample of scores about the levels of burnout from six nurses. We were able to calculate that the sample mean was 106. However, we know that this mean is only an estimate of the true population mean burnout score in the population of nurses, which is the value of true interest. It is unlikely that we would ever be able to know the average value of burnout in the whole population so we have to rely on estimates from smaller samples as the best approximation.
---
...
What we have to keep in mind though, is that not all samples are created equal. Some are better estimates than others. Consider this example of the relationship between sample size and variability of the mean. In the long run, large samples are necessary to get an accurate estimate of the mean of the population. But how can we quantify the accuracy of the estimate of a mean, given its sample size?
...
```{r fig.show="animate", animation.hook = 'gifski', fig.width=7, fig.height=3, echo=FALSE, message=FALSE, warning=FALSE, results = 'hide', interval=0.08, loop=TRUE, cache=TRUE}
library(ggplot2)
library(tweenr)
library(parallel)
library(MASS)
...
One way to illustrate this is to generate data not just from a single sample but from many samples of the same size drawn from the same population. The animation here illustrates the process of creating the sampling distribution for one thousand sample means of from nurses who rated their confidence in leadership skills on an instrument that ranged from 0 to 15. Each sample consists of fifty nurses, and we're flashing through one thousand samples in total. In the histogram on the left, each dot signifies one of the fifty observations in the sample. After each sample histogram is completed, the mean is calculated. This mean value is then subsequently added to the histogram of the sampling distribution on the right. As you can see, this process is repeated, allowing the sampling distribution to build up. Looking at the spread of mean leadership values on the right, we can get a sense of the range of many possible estimates of the population mean that a sample of fifty nurses can produce. Looking at the spread of mean values that were generated, we can get a sense of the range of many possible estimates of the population mean that a sample of 50 nurses can produce. Notice that the range of possible mean values were between 8.9 and 10.7. This means that mean values outside of this range are essentially improbable. 
---
class: top, left

# Sample size and sampling distribution

```{r, tut=FALSE, echo=FALSE, message= FALSE, warning=FALSE, fig.height=3, fig.width=3, cache = TRUE}
library(ggplot2)
samp <- function(n){
  x <- rnorm(n, 9.7, 2.1)
  m <- mean(x)
...
Back to the idea that larger samples are better than smaller ones, we can explore what happens if we redo this sampling experiment, this time sampling five hundred nurses instead of fifty before taking the mean each time, and repeating this until thousands of sample mean values have been recorded. For completeness, let’s also do the same with samples of five nurses each. We compare the resulting sampling distributions from all three scenarios here, with the plot in the middle corresponding to the sampling distribution we already generated in the previous animation. What do we notice? Well, for one thing, all the histograms look like a normal distribution with that characteristic bell curve shape. We can also notice that the distributions generated from larger samples are less dispersed than the one with just 5 nurses in each sample down the bottom.
